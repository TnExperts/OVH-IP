...
// You have to get OWN Credentials for the API: https://eu.api.soyoustart.com/createToken/index.cgi?GET=/*
...
foreach($regions AS $region) {
	...
	foreach($blocksizes AS $blocksize) {
		...
		curl_setopt($conn, CURLOPT_URL, "https://eu.api.soyoustart.com/1.0/auth/time"); 
		curl_setopt($conn, CURLOPT_RETURNTRANSFER, 1); 
		$timestamp = curl_exec($conn); 
 
		$url = 'https://eu.api.soyoustart.com/1.0/order/dedicated/server/'.$servicename.'/ip?country='.$region.'&organisationId='.$organisationID.'&blockSize='.$blocksize.'&type=failover';
